...
class Game(Frame):
    ...
    def setupGUI(self):
        ...
        Game.player_input.grid(row=2, column=2, sticky=N+S+E+W)
        Game.player_input.focus()


        img = None
        Game.image = Label(self, width = WIDTH, image = img)
        Game.image.grid(row=1, column=2, sticky=N+S+E+W)
        Game.image.pack_propagate(False)

        #text for now
        text_frame = Frame(self, width = int(WIDTH/2))
        Game.text = Text(text_frame, bg = "lightgrey", state = DISABLED)
        Game.text.grid(row=1, column=1, sticky=N+S+E+W)
        text_frame.grid(row=2, column=1, sticky=N+S+E+W)
